...
const regexMain = /    const isImperial = unit === 'in';[\s\S]*?const sheetAreaSqFt = \(mW \* mH\) \/ conversionFactor;\s*const sheetsNeeded = \(sheetAreaSqFt > 0\) \? Math\.ceil\(totalRequiredSqFt \/ sheetAreaSqFt\) : 0;\s*totalCost = sheetsNeeded \* pricePerSheet;\s*suggestedOrderText = \`\$\{sheetsNeeded\} Sheets\`;\s*unitCostText = \`\$\{Number\(pricePerSheet \|\| 0\)\.toFixed\(2\)\} \/ sheet\`;\s*\}/g;

const replacementMain = `    const isImperial = unit === 'in';
...
code = code.replace(regexMain, replacementMain);

const regexPaint = /        const isImperial = unit === 'in';\s*const baseSqFt = isImperial \? \(g\.netArea \/ 144\) : \(g\.netArea \/ 929\.0304\);\s*const totalRequiredSqFt = baseSqFt \* \(1 \+ \(overage \/ 100\)\);[\s\S]*?unitCostText = \`\$\$\{Number\(pricePerSheet \|\| 0\)\.toFixed\(2\)\} \/ sheet\`;\s*\}/g;

const replacementPaint = `        const isImperial = unit === 'in';
        const conversionFactor = isImperial ? 144 : 929.0304;
...
code = code.replace(regexPaint, replacementPaint);
