...
export const useCloudAutoSave = () => {
  ...
  const getSnapshot = () => {
    return {
      ...
      disableColorWithTexture: disableColorWithTexture ?? false,
      textureOpacity: textureOpacity ?? 0.8,
      textureScale: textureScale ?? 1.0,
      textureScaleRandom: textureScaleRandom ?? false,
      textureRotationMode: textureRotationMode || 'random',
      textureRotationAngle: textureRotationAngle ?? 0,
